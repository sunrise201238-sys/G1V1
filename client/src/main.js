...
function setupHUD() {
  ...
  window.addEventListener('pointermove', (e) => {
    ...
    applyStick(e.clientX, e.clientY);
    if (Math.hypot(input.x, input.y) > 0.15 && input.boostPreserved) input.boost = true;
  });
  ...
  hud.querySelectorAll('button').forEach((btn) => {
    ...
    btn.addEventListener('pointerup', () => {
      if (k === 'shoot') input.shootHold = false;
        ...
        input.boostHeld = false;
        const directionHeld = Math.hypot(input.x, input.y) > 0.15;
        input.boost = directionHeld;
        input.boostPreserved = directionHeld;
      }
      else if (k === 'step') input.step = false;
...
function updateBoost(mech, now, action) {
  ...
  else if (grounded) s.boost = Math.min(100, s.boost + 5.4); // no dead-time refill while grounded
...
function updatePlayer(now) {
  ...
  if (input.boostPreserved && (input.boostHeld || directionHeld)) input.boost = true;
  if (!input.boostHeld && !directionHeld) {
    input.boost = false;
    input.boostPreserved = false;
  }
  ...
  const speed = input.boost ? 21 : (recoveringFromDash ? 6.5 : 10);
  const hitStunScale = now < state.player.state.hitStunUntil ? 0.25 : 1;
  if (!inMeleeLunge) {
    state.player.body.velocity.x = move.x * speed * hitStunScale;
    state.player.body.velocity.z = move.z * speed * hitStunScale;
  }
  ...
  if (input.rise) {
    ...
    inheritMomentum(state.player, 0.82);
    action = 'rise';
  } else if (input.step) {
    input.boost = false;
    input.boostPreserved = false;
    ...
    inheritMomentum(state.player, 0.7);
    action = 'step';
  } else if (input.boost) {
    state.player.state.evadeHomingUntil = now + 320;
    state.player.state.antiMeleeUntil = now + 260;
    inheritMomentum(state.player, 0.9);
    action = 'dash';
  ...
  if (input.meleeTap) {
    ...
    if (state.player.state.redLock && now >= state.player.state.meleeCooldownUntil) {
      ...
      state.player.state.meleeLungeVX = lunge.x * 44;
      state.player.state.meleeLungeVZ = lunge.z * 44;
      state.player.body.velocity.x = state.player.state.meleeLungeVX;
      state.player.body.velocity.z = state.player.state.meleeLungeVZ;
