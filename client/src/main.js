...
const input = { x: 0, y: 0, boost: false, rise: false, step: false, shootTap: false, shootHold: false, meleeTap: false };
  ...
  hud.querySelectorAll('button').forEach((btn) => {
    ...
    btn.addEventListener('pointerup', () => {
      if (k === 'shoot') input.shootHold = false;
        ...
        if (Math.hypot(input.x, input.y) < 0.2) input.boost = false;
      } else if (k === 'step') {
        if (Math.hypot(input.x, input.y) < 0.2) input.step = false;
...
function updateBoost(mech, now, action) {
  ...
  else if (grounded && !['shoot', 'melee-lunge'].includes(action)) s.boost = Math.min(100, s.boost + 4.8); // instant-feel refill on ground/basic movement
...
function updatePlayer(now) {
  ...
  if (input.meleeTap) {
    if (state.player.state.redLock && now >= state.player.state.meleeCooldownUntil) {
      ...
      state.player.body.velocity.x = lunge.x * 38;
      state.player.body.velocity.z = lunge.z * 38;
      state.player.state.meleeAnimUntil = now + 220;
      state.player.state.meleeLungeUntil = now + 260;
      state.player.state.meleeStrikeUntil = now + 260;
